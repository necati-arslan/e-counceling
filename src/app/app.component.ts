...
export class AppComponent implements OnInit, OnDestroy {
  ...
  constructor(private authService: AuthService,
    ...
    router.events.subscribe((e: any) => {
      if (e instanceof NavigationEnd) {
        console.log(e)
        if (e.url == '/' || e.url.startsWith("/#")) { this.currentUrl = true; }
        else {
  ...
  checkLastSeansDuration(userType,userId) {
    this.roomService.getLastSeans(userType, userId).subscribe((lastSeans: any) => {
      ...
      setTimeout(() => {
        ...
        this.roomService.getRoomById(lastSeans.roomId).pipe(take(1)).subscribe((room: any) => {
  ...
  checkReservedAppointment(userId) {
    this.roomService.updateAvaible(this.userId, true);//kontrol
